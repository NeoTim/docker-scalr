...
if [[ ! -d /opt/scalr-server ]]; then
    mkdir -p /opt/scalr-server/var/log/supervisor
fi
if [[ ! -f /etc/scalr-server/scalr-server.rb ]]; then
    echo "BOOTSTRAP - Generating default settings"
    cp /etc/scalr-server-template/scalr-server.rb /etc/scalr-server/scalr-server.rb
...
if [[ ! -f /etc/scalr-server/scalr-server-secrets.json ]]; then
    ...
    cp /etc/scalr-server-template/scalr-server-secrets.json /etc/scalr-server/scalr-server-secrets.json
...
if [[ ! -f /opt/scalr-server/version-manifest.json ]]; then
    ...
    cp /opt/scalr-server-template/* /opt/scalr-server
fi
